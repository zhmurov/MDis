...
FILE* dcd_open_write(FILE* dcd_file, char *dcd_filename){
	dcd_file = safe_fopen(dcd_filename, "w");
	return dcd_file;
}

FILE* dcd_open_append(FILE* dcd_file, char *dcd_filename){
	dcd_file = safe_fopen(dcd_filename, "a");
	return dcd_file;
}
...
FILE* dcd_open_read(FILE* dcd_file, char *dcd_filename){
	dcd_file = safe_fopen(dcd_filename, "r");
	return dcd_file;
}
...
void dcd_read_header(FILE* dcd_file, int* N, int* NFILE, int* NPRIV, int* NSAVC, float* DELTA){
	...
	safe_fread(NFILE, 4, 1, dcd_file);
	safe_fread(NPRIV, 4, 1, dcd_file);
	safe_fread(NSAVC, 4, 1, dcd_file);
	safe_fread(&iin, 4, 1, dcd_file);
	for(int i = 0; i < 5; i++){
		safe_fread(&iin, 4, 1, dcd_file);
	}
	safe_fread(DELTA, 4, 1, dcd_file);
	for(int i = 0; i < 9; i++){
		safe_fread(&iin, 4, 1, dcd_file);
	}
	//24
	...
	safe_fread(&iin, 4, 1, dcd_file);
	char title[80];
	safe_fread(&title, 80, 1, dcd_file);
	//printf("Title1: %s\n", title);
	safe_fread(&title, 80, 1, dcd_file);
	//printf("Title2: %s\n", title);
	//164
...
int dcd_read_frame(FILE* dcd_file, int N, float *X, float *Y, float *Z){
	...
	safe_fread(&iin, 4, 1, dcd_file);
	//printf("%d\n", iin);
	safe_fread(X, 4*N, 1, dcd_file);
	safe_fread(&iin, 4, 1, dcd_file);
	//printf("%d\n", iin);
	safe_fread(&iin, 4, 1, dcd_file);
	//printf("%d\n", iin);
	safe_fread(Y, 4*N, 1, dcd_file);
	safe_fread(&iin, 4, 1, dcd_file);
	//printf("%d\n", iin);
	safe_fread(&iin, 4, 1, dcd_file);
	//printf("%d\n", iin);
	safe_fread(Z, 4*N, 1, dcd_file);
	safe_fread(&iin, 4, 1, dcd_file);
	//printf("%d\n", iin);
	if(feof(dcd_file) == 0){
		return 0;
